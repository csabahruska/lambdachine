...
type HHsc a = IO (Messages, Maybe a)

toHHsc :: IO a -> IO (Messages, Maybe a)
toHHsc io = do a <- io; return ((emptyBag, emptyBag), Just a)
...
defaultHooks :: FileHooks
defaultHooks = FileHooks
  { hookParse = id
  , hookTypecheckRename = id
  , hookDesugar = id
  , hookOptimize = id
  , hookWriteIface = id
  , hookCodeGen = id
  , hookPostBackendPhase = id
  }
...
hscTypecheckRename :: HscEnv -> FileHooks -> ModSummary -> HsParsedModule
                   -> IO (TcGblEnv, RenamedStuff)
hscTypecheckRename hsc_env hooks mod_summary rdr_module = runHsc hsc_env $ do

    tc_result <- tcRnModule' hsc_env hooks mod_summary True rdr_module
...
tcRnModule' :: HscEnv -> FileHooks -> ModSummary -> Bool -> HsParsedModule
            -> Hsc TcGblEnv
tcRnModule' hsc_env hooks sum save_rn_syntax mod = do
    tcg_res <- {-# SCC "Typecheck-Rename" #-}
               ioMsgMaybe $
                 hookTypecheckRename hooks
                   (\hsc_env mod_summary rdr_module ->
                       tcRnModule hsc_env (ms_hsc_src mod_summary)
                                  save_rn_syntax rdr_module)
                   hsc_env sum mod
...
genericHscRecompile :: HsCompiler a -> FileHooks
                    -> ModSummary -> Maybe Fingerprint
                    -> Hsc a
genericHscRecompile compiler hooks mod_summary mb_old_hash
    | ExtCoreFile <- ms_hsc_src mod_summary =
        panic "GHC does not currently support reading External Core files"
    | otherwise = do
        tc_result <- hscFileFrontEnd hooks mod_summary
        hscBackend compiler tc_result mod_summary mb_old_hash
...
hscOneShotCompiler :: FileHooks -> HsCompiler OneShotResult
hscOneShotCompiler hooks = HsCompiler {
  ...
  , hscRecompile = genericHscRecompile (hscOneShotCompiler hooks) hooks

  , hscBackend = \tc_result mod_summary mb_old_hash -> do
        dflags <- getDynFlags
        case hscTarget dflags of
            HscNothing -> return (HscRecomp Nothing ())
            _otherw    -> genericHscBackend (hscOneShotCompiler hooks)
  ...
  , hscGenOutput = \guts0 mod_summary mb_old_iface -> do
        env <- getHscEnv
        guts <- ioMsgMaybe $ hookOptimize hooks ((`unHsc` env) . hscSimplify') guts0
        ...
        mbStub <- ioMsgMaybe $ hookCodeGen hooks
                    (\iface' details' cgguts' mod_summary' -> do
                        unHsc (hscGenHardCode cgguts' mod_summary') env')
                    iface details cgguts mod_summary

        -- hasStub <- hscGenHardCode cgguts mod_summary
        return (HscRecomp mbStub ())
  }

-- Compile Haskell, boot and extCore in OneShot mode.
hscCompileOneShot :: FileHooks -> Compiler OneShotResult
hscCompileOneShot hooks hsc_env mod_summary src_changed mb_old_iface mb_i_of_n
  = do
    ...
    genericHscCompile (hscOneShotCompiler hooks)
                      oneShotMsg hsc_env' mod_summary src_changed
                      mb_old_iface mb_i_of_n

hscOneShotBackendOnly :: FileHooks -> TcGblEnv -> Compiler OneShotResult
hscOneShotBackendOnly hooks = compilerBackend (hscOneShotCompiler hooks)
...
hscFileFrontEnd :: FileHooks -> ModSummary -> Hsc TcGblEnv
hscFileFrontEnd hooks mod_summary = do
    hpm <- hscParse' mod_summary
    hsc_env <- getHscEnv
    tcg_env <- tcRnModule' hsc_env hooks mod_summary False hpm
    return tcg_env
...
{-
import HscMain ( HscStatus'(..), HscStatus )
import HscTypes
import Data.IORef
import NameEnv          ( emptyNameEnv )
import MkIface          ( recompileRequired, checkOldIface, RecompileRequired(..) )
import ErrUtils
import MonadUtils
import Bag
import DynFlags
...
hscCompileOneShot hsc_env0 mod_summary source_modified mb_old_iface0 mb_mod_index
  = do
    -- One-shot mode needs a knot-tying mutable variable for interface
    -- files. See TcRnTypes.TcGblEnv.tcg_type_env_var.
    type_env_var <- newIORef emptyNameEnv
    let mod = ms_mod mod_summary
        hsc_env = hsc_env0{ hsc_type_env_var = Just (mod, type_env_var) }
...
-}
    ...
    let skip iface = do
            hscMessage hsc_env mb_mod_index UpToDate mod_summary
            runHsc hsc_env $ do  -- hscNoRecomp compiler iface
              hsc_env <- getHscEnv
              liftIO $ dumpIfaceStats hsc_env
              return HscNoRecomp
...
-- | Report statistics on reading interfaces.
dumpIfaceStats :: HscEnv -> IO ()
dumpIfaceStats hsc_env = do
    eps <- readIORef (hsc_EPS hsc_env)
    dumpIfSet dflags (dump_if_trace || dump_rn_stats)
