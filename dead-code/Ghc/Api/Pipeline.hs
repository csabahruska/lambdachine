...
import Util ( Suffix, reslash, Direction(..), createDirectoryHierarchy )
...
data PhaseImplementations m = PhaseImplementations
  { runUnlit :: DynFlags -> [String] -> InputFilePath -> OutputFilePath -> m ()
    ...
    --
  , runHsCompiler :: HscEnv -> ModSummary -> SourceUnchanged -> m HscStatus }
...
defaultRunUnlit :: GhcMonad m =>
     DynFlags -> [String] -> InputFilePath -> OutputFilePath -> m ()
...
defaultRunHsCompiler :: GhcMonad m =>
     HscEnv -> ModSummary -> SourceUnchanged -> m HscStatus
